...
- `nimToken` 文档和规范现推荐使用 `appKey|accid|token`；旧格式 `appKey-accid-token` 继续保留兼容

## [1.0.3] - 2026-04-10

### 修复

- 修复 OpenClaw 2026.4.9 上安装插件失败的问题：插件 API 版本兼容性检查现使用 semver 范围（`^2026.4.5`）而非精确版本，允许与更新的 OpenClaw 补丁版本向前兼容
...
- `nimToken` 三合一凭证配置：支持推荐格式 `appKey|accid|token` 和旧格式 `appKey-accid-token`，简化配置流程，优先于独立的 `appKey`/`account`/`token` 字段（推荐）
...
- **破坏性变更**：推荐使用 `nimToken` 三合一配置（`appKey|accid|token`，兼容旧 `appKey-accid-token`）；独立的 `appKey`/`account`/`token` 字段已弃用但仍可用
