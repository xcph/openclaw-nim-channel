...
### 变更
- `SenderId` 改为使用显示名称（昵称）替代原始 accid —— 有昵称时仅显示昵称，无昵称时回退到 accid；适用于所有消息类型（单聊、群聊、圈组）

## [0.4.1] - 2026-03-19

### 新增
- **安全通配置**：NIM 配置新增 `antispamEnabled` 字段 — 控制出站文本和回复消息的 NIM 服务端内容审核（安全通），默认为 `true`
- 圈组频道主题（subject）现已包含在 Agent 提示词上下文中

### 变更
- 全代码库应用 Prettier 自动格式化（新增 `.prettierrc`）
- 代码风格统一规范化 — 除格式化外无功能性变更
