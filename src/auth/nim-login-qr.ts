...
type NimQrResolved = NimQrLoginConfig & {
  baseUrl: string;
...
function buildIlinkFetchHeaders(skRouteTag: string): Record<string, string> {
  return {
    ...
    "User-Agent": "OpenClaw-NIM-QR/1.0",
  };
}
...
/** 将纯 base64 图片内容包装为 data URL，便于客户端展示。 */
...
export function resolveNimQrLoginFromConfig(cfg: OpenClawConfig): NimQrResolved | null {
  ...
  const q = nim?.qrLogin;
  if (!q?.botType?.trim() || !q?.appKey?.trim()) {
    return null;
  }
...
export async function startNimLoginWithQr(params: {
  ...
  if (!qrCfg) {
    return {
      message:
        "未配置 NIM 扫码登录：请在 openclaw.json 的 channels.nim.qrLogin 中设置 botType 与 appKey（用于 ilink 兼容接口）。",
      sessionKey,
    };
  }
  ...
  try {
    ...
    if (!qrResponse.qrcode) {
      return {
        message:
          "ilink 未返回二维码令牌（qrcode）。请核对 channels.nim.qrLogin.botType、baseUrl，必要时调整 skRouteTag。",
        sessionKey,
      };
    }
    if (!qrResponse.qrcode_img_content) {
      return {
        message:
          "ilink 未返回二维码展示链接或图片字段（如 qrcode_img_content）。请核对接入环境与 botType，或在 qrLogin 中设置 skRouteTag。",
        sessionKey,
      };
    }
    ...
    return {
      ...
      message: "请使用移动端扫描以下二维码，完成 NIM 机器人登录。",
      sessionKey,
    };
  } catch (err) {
    return {
...
export async function waitForNimLogin(params: {
  ...
  if (!qrCfg) {
    return { connected: false, message: "未配置 channels.nim.qrLogin，无法等待扫码。" };
  }

  if (!activeLogin) {
    return { connected: false, message: "当前没有进行中的登录，请先发起 nim-web.login.start。" };
  }
